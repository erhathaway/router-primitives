...
describe('Router', () => {
    ...
    describe('Is a path router', () => {
        describe('True', () => {
            ...
            it('Parent is a path router and type is "scene" with a data neighbor NOT set to be a path router', () => {
                const dataRouterOne = new (Router as any)(
                    generateMockInit({name: 'data1', type: 'data', config: {isPathRouter: false}})
                );
                const dataRouterTwo = new (Router as any)(generateMockInit({type: 'data'}));
                const sceneRouter = new (Router as any)(
                    generateMockInit({name: 'scene1', type: 'scene'})
                );
            ...
            it('Parent is a path router and type is "data" with no scene neighbors', () => {
                const dataRouterOne = new (Router as any)(
                    generateMockInit({name: 'data1', type: 'data', config: {isPathRouter: true}})
                );
                const dataRouterTwo = new (Router as any)(generateMockInit({type: 'data'}));
                const sceneRouter = new (Router as any)(
                    generateMockInit({name: 'scene1', type: 'feature'})
                );
                const parentRouter = new (Router as any)(generateMockInit());
        ...
        describe('False', () => {
            it('Parent is a path router and type is "scene" with a data neighbor set to be a path router', () => {
                ...
                const dataRouterTwo = new (Router as any)(
                    generateMockInit({type: 'data', config: {isPathRouter: true}})
                );
                const sceneRouter = new (Router as any)(
                    generateMockInit({name: 'scene1', type: 'scene'})
                );
    ...
    describe('Route key', () => {
        ...
        it('Returns the route key if one was set during initalization', () => {
            const dataRouterOne = new (Router as any)(
                generateMockInit({name: 'data1', type: 'data', config: {routeKey: 'hello'}})
            );
